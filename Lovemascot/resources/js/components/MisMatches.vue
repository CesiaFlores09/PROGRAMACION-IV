    ...
    <div>
        <div class="card">
            <div class="card-header center-content">
                <p>Matches recibidos</p>
                <input type="text" v-model="buscar" placeholder="Buscar..." class="form-control" @keyup="buscarMascota()">
            </div>
            <div class="card-body">
                <div v-for="mascota in recibidas" class="row border-2 border-bottom border-secondary pb-2 rounded center-content hover-container d-flex justify-content-between align-items-center" :key="mascota.id" :class="{'border border-success': mascota.actual, 'd-none': !mascota.mostrar}">
                    <div class="row">
        ...
        <div class="card">
            <div class="card-header center-content">
                <p>Matches enviados</p>
                <input type="text" v-model="buscar" placeholder="Buscar..." class="form-control" @keyup="buscarMascota()">
            </div>
            <div class="card-body">
                <div v-for="mascota in enviadas" class="row border-2 border-bottom border-secondary pb-2 rounded center-content hover-container d-flex justify-content-between align-items-center" :key="mascota.id" :class="{'border border-success': mascota.actual, 'd-none': !mascota.mostrar}">
                    <div class="row">
                        <div class="col-md-6 center-content">
                            <p><b>{{ mascota.nombre }}</b></p>
                            <img :src="'storage/imagenes/mascotas/fotos/'+mascota.imagen" alt="" class="img-fluid" style="max-width: 100%;">
                ...
                axios[metodo](url, datos)
                    .then(response => {
                        ...
                        if (metodo != 'delete') {
                            ...
                            this.enviadas = this.mascotas.filter(mascota => mascota.duenio != this.usuario.id && mascota.match == 'enviado' && mascota.estado == 0);
                            this.recibidas = this.mascotas.filter(mascota => mascota.duenio != this.usuario.id && mascota.match == 'recibido' && mascota.estado == 0);
                            this.misMascotas = this.mascotas.filter(mascota => mascota.duenio == this.usuario.id);
                        }
                    })
                    .catch(error => {
                        console.log(error);
